...
def assert_barcode(barcode, msg = nil)
  full_message = message(msg) do
    "Expected #{mu_pp(barcode)} to have a valid PDF417 start and end sequence for each row"
  end
  assert (barcode.encoding.any? && barcode.encoding.all? do |x|
            x.start_with?('11111111010101000') && x.end_with?('111111101000101001')
          end), full_message
end
