...
class PDF417
  # Main library functions of PDF417
  class Lib
    ...
    def inspect # :nodoc:
      ...
      "#<#{self.class.name}:#{format('0x%x', object_id)} #{attributes.join(' ')}>"
    end

    private

    def inspect_attributes
      %i[text bit_columns bit_length code_rows code_columns codeword_length error_level generation_options
         aspect_ratio y_height generation_error]
    end
  end
end
